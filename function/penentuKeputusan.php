...
$sqlSelectDistinctVariabel = mysql_query("SELECT DISTINCT(variabel) FROM pohon_keputusan");
...
while($rowDataPenentuKelancaran = mysql_fetch_array($sqlDataPenentuKelancaran)) {
    // ambil variabel dan nilai variabel pada db rule_prediksi
    ...
    while($rowRulePenentuKelancaran = mysql_fetch_array($sqlRulePenentuKelancaran)) {
        if (!empty($rowRulePenentuKelancaran)) {
            // jika variabel pada db data_variabel_prediksi sama dengan variabel pada db rule_prediksi
            if ($rowRulePenentuKelancaran['variabel'] == $rowDataPenentuKelancaran['variabel']) {
                // jika nilai variabel pada db data_variabel_prediksi sama dengan nilai variabel pada db rule_prediksi
...
$sqlKelancaran = mysql_query("SELECT distinct id_rule, kelancaran FROM `rule_prediksi` WHERE pohon = 'C45' AND cocok = 'Ya' and id_rule not in (select distinct id_rule from `rule_prediksi` where pohon = 'C45' AND cocok = 'Tidak') AND kelancaran IN ('Ya','Tidak')");
...
if (!empty($rowKelancaran)) {
    ...
    if ($kelancaran == 'Ya') {
        ...
        echo "<script>document.location.href='../index.php?p=prediksi&id=$no_anggota';</script>\n";
